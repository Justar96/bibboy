...
const ActivityGroupBlock = memo(function ActivityGroupBlock({
  ...
  return (
    <div className="border-b border-paper-300 last:border-b-0">
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-1.5 px-2 py-1.5 text-left hover:bg-paper-200 transition-colors"
      >
        <ChevronIcon
        ...
        />
        <span className="font-mono text-[10px] text-ink-400 leading-tight truncate flex-1">
          {truncate(group.userText)}
        </span>
        {activeCount > 0 && (
          <span className="w-1.5 h-1.5 rounded-full bg-[#6B9FFF] animate-pulse shrink-0" />
        )}
        <span className="font-mono text-[9px] text-ink-300 shrink-0">{group.actions.length}</span>
      </button>
        ...
        <div className="pb-1.5">
          {group.actions.length === 0 ? (
            <div className="px-5 py-2">
              <span className="font-mono text-[9px] text-ink-300">Waiting for activity…</span>
            </div>
          ) : (
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-3 py-2 border-b border-paper-300 flex items-center gap-2 bg-paper-100">
        ...
        {running > 0 && (
          <span className="ml-auto font-mono text-[9px] font-semibold text-white bg-[#6B9FFF] rounded-full min-w-[16px] h-4 flex items-center justify-center px-1">
            {running}
          </span>
        )}
      </div>
      ...
      {reversed.length === 0 ? (
        <div className="flex-1 px-2 py-4 text-center">
          <span className="font-mono text-[10px] text-ink-300">No activity yet</span>
        </div>
      ) : (
