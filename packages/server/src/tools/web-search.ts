...
function parseBraveSearchResponse(data: unknown): BraveSearchResult[] | null {
  ...
  const web = dataRecord.web;
  let rawResults: unknown[] = [];

  if (web !== undefined) {
    const webRecord = toRecord(web);
    if (!webRecord) {
      return null;
    }

    rawResults = toArray(webRecord.results) ?? [];
  }
  ...
  for (const entry of rawResults) {
    ...
    results.push({
      title: toStringValue(entryRecord.title) ?? undefined,
      url: toStringValue(entryRecord.url) ?? undefined,
      description: toStringValue(entryRecord.description) ?? undefined,
      age: toStringValue(entryRecord.age) ?? undefined,
    });
  }
...
async function runPerplexitySearch(params: {
  ...
  function parsePerplexityResponse(data: unknown): ParsedPerplexityResponse | null {
    ...
    const choices = dataRecord.choices === undefined ? [] : toArray(dataRecord.choices);
    if (choices === null) {
      return null;
    }

    const citationsRaw = dataRecord.citations === undefined ? [] : toArray(dataRecord.citations);
    if (citationsRaw === null) {
      return null;
    }

    const firstChoiceRecord = toRecord(choices[0]);
    const firstChoiceMessage = firstChoiceRecord ? toRecord(firstChoiceRecord.message) : null;
    let answer = "";
    if (firstChoiceMessage) {
      answer = toStringValue(firstChoiceMessage.content) ?? "";
    }

    const citations: string[] = citationsRaw
      .map((citation) => toStringValue(citation))
...
function parseSearchProvider(value: string | undefined, fallback: SearchProvider): SearchProvider {
    ...
    execute: async (_toolCallId, args): Promise<ToolExecutionResult> => {
      ...
      if (!hasAnyProvider) {
        const result: WebSearchResult = {
          query: toStringValue(args.query) ?? "",
          provider: "brave",
          count: 0,
          tookMs: 0,
          results: [],
      ...
      try {
        ...
        const provider = parseSearchProvider(
          readStringParam(args, "provider") || undefined,
          defaultProvider,
        );
